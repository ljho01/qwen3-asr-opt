...
from qwen_asr_opt.selective_precision import checkpoint_arrays, quantize_selective
...
def test_only_decoder_linears_change_and_protected_tensors_stay_exact(bits):
    model, baseline = pair()
    configs = quantize_selective(model, bits)
    assert sum(c["bits"] == bits for c in configs.values()) == 14
    assert all(c["bits"] == 8 for p, c in configs.items() if not p.startswith("model.layers."))
...
def test_requantizing_q8_and_unsupported_precision_are_rejected():
    ...
    with pytest.raises(ValueError, match="original unquantized"):
        quantize_selective(baseline, 6)
    with pytest.raises(ValueError, match="5or6"):
        quantize_selective(model, 4)


def test_invalid_tied_head_is_not_silently_aliased():
    model, _ = pair()
    quantize_selective(model, 6)
